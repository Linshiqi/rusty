...
#[component]
fn EditorSettings() -> impl IntoView {
    let state = AppState::expect();

    view! {
        <Field
            label="Vim keys"
            help="Modal editing in the editor: normal, insert and visual modes, with the                   mode shown in the status bar. Remembered across launches and shared by                   every window. Chords stay with the editor — Ctrl+S saves, Ctrl+A selects                   all, Ctrl+C copies — and insert mode behaves exactly as it does with this                   off, so nothing you already know stops working."
        >
            <div class="inline-flex self-start rounded-[7px] bg-sunken p-0.5">
                {[("Off", false), ("On", true)]
                    ...
                    .map(|(label, wanted)| {
                        let class = move || {
                            let on = state.vim_on.get() == wanted;
        ...
        <Field
            label="Text size"
            help="The editor's own zoom, separate from the interface scale in Appearance.                   Ctrl+= and Ctrl+- change it from the editor too."
        >
            <div class="flex items-center gap-3">
                            ...
                            <button
                                ...
                                on:click=move |_| {
                                    let next = if step == 0.0 {
                                        ...
                                        (state.editor_zoom.get_untracked() + step * 0.1)
                                            .clamp(0.6, 2.4)
                                    };
                                    state.editor_zoom.set(next);
                ...
                <span class="tnum font-mono text-caption text-label-3">
                    {move || format!("{:.0}%", state.editor_zoom.get() * 100.0)}
                </span>
            </div>
        </Field>
    }
...
#[component]
fn UpdateSettings() -> impl IntoView {
    let state = AppState::expect();

    view! {
        <Field
            label="Version"
            help="Installing a new one is a download today. In-place updates                   need the release to be signed; the release workflow emits the                   signed feed as soon as the keypair exists."
        >
            <div class="flex flex-col gap-3">
                <div class="flex items-center gap-3">
                    <code class="rounded-[4px] bg-sunken px-1.5 py-0.5 font-mono text-footnote">
