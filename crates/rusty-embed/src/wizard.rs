...
/// Options `esp-generate` understands, with what each one costs.
///
...
pub fn options() -> Vec<(String, String, String)> {
    OPTIONS
        .iter()
        .map(|(id, label, detail)| (id.to_string(), label.to_string(), detail.to_string()))
        .collect()
}
...
pub fn explain(choice: &WizardChoice) -> Vec<Explanation> {
    ...
    for option in &choice.options {
        if let Some((_, label, detail)) = OPTIONS.iter().find(|(id, _, _)| id == option) {
            out.push(Explanation {
